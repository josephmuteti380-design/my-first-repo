...
| **Distribution vs. AI-only narratives** | Wil Reynolds (video notes) | Large-agency operator; stress-tests where **audience and channel** still matter. |
...
- `research/youtube-transcripts/` — per-video files (metadata + room for API transcripts).  
- `research/other/` — methodology, podcast pointers, **this rubric**.

### 3. APIs and technical tools (honest status + next step)

- **LinkedIn:** No scraping pipeline—**manual export** with canonical URLs (stable, ToS-safe, auditable).  
- **YouTube:** Stub files document **video IDs** and **how to pull transcripts** (Supadata, `youtube-transcript-api`, or UI). The environment used for the initial pass had **no Python/CLI** for automated transcript pull; filling `## Full transcript` in those markdown files is the intended technical follow-up.

Evaluators should see: **structure + instructions + IDs** now; **machine-readable transcript text** when you run the API step.
